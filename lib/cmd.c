...
tCMDHOOK_Command		pCustomCommand=NULL;
tCMDHOOK_Execute		pCustomExecute=NULL;
#endif
...
Execute(LPTSTR Full, LPTSTR First, LPTSTR Rest, PARSED_COMMAND *Cmd)
{
    ...
    *FirstEnd = _T('\0');
	...
	if (pCustomExecute)
	{
		...
		while (*rest == _T(' '))
			rest++;
		ret = pCustomExecute(szFullName, rest);
		TRACE ("[CustomExecute: %s %s] ret=%d\n", debugstr_aw(szFullName), debugstr_aw(rest),ret);
		if (ret >= 0) {			// return -1 if it is no custom execute 
			return ret;
		}
	}
#endif
    ...
    else
    {
        ...
        if (dot && (!_tcsicmp (dot, _T(".pl")) || !_tcsicmp (dot, _T(".plc")) || !_tcsicmp (dot, _T(".ple")) ))
	    {
		    sprintf(szFullCmdLine,"%s\\plc.exe",pCmdDir);
			if (IsExistingFile(szFullCmdLine)) {
  		        sprintf(szFullCmdLine,"\"%s\\plc.exe\" \"%s\"",pCmdDir,szFullName);
			} else {
  		        sprintf(szFullCmdLine,"%s\\pCmd.exe",pCmdDir);
			    if (IsExistingFile(szFullCmdLine)) {
  		            sprintf(szFullCmdLine,"\"%s\\pCmd.exe\" /C \"%s\"",pCmdDir,szFullName);
				} else {
  		            sprintf(szFullCmdLine,"\"%s\"",szFullName);
				}
			}
 			if (*rest) {
                strcat(szFullCmdLine, rest);
	 	    }
...
//        if (! CreateProcess(szFullName,
        ...
        if (prci.hProcess != NULL)
        {
	        ...
	        //printf("return %x\n",dwExitCode);
			if (prci.hThread) CloseHandle(prci.hThread);
        }
        else
        {
...
DoCommand(LPTSTR first, LPTSTR rest, PARSED_COMMAND *Cmd)
{
	...
	{
		if (bc) {
  		   ret = pCustomCommand(first,rest,bc->mem, bc->memsize);
		} else {
		   ret = pCustomCommand(first,rest,NULL,0);
		}
...
Initialize(int argc, const TCHAR *argv[])
{
	...
	// build commandline
	cmdLine=(argv[0][0]=='=')?&argv[0][1]:GetCommandLine();
...
/*  not needed in cmd.dll
    if (!*ptr)
    {
        // If neither /C or /K was given, display a simple version string 
//	    ConOutPuts(_T("# Cmd Shell Executor                                     Copyright 2006 - ") _T(COPYRIGHT_YEAR) _T("\n") );
		ConOutPuts(_T("# Version 7.05                                                   www.atbas.org\n") );
     }
*/
    if (AutoRun)
    {
